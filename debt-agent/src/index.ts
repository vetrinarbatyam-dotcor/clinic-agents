...
async function processEscalations(config: Record<string, string>): Promise<{
  ...
  for (const debt of activeDebts as DebtRecord[]) {
    ...
    if (decision.autoSend && !DRY_RUN) {
      ...
      // Only bump escalation if message was actually sent
      if (result.sent) {
        await pool.query(
        ...
        console.warn(`[debt-agent] WhatsApp failed for ${debt.user_id}, keeping escalation at level ${debt.escalation_level}`);
      }
    } else {
...
async function main() {
  ...
  try {
    ...
    } catch (e) { console.error("[debt-agent] failure alert WhatsApp failed:", e instanceof Error ? e.message : e); }
    process.exit(1);
  } finally {
    await pool.end();
  }
